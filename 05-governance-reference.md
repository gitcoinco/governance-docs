...
Gitcoin protocol is goverened and upgraded by GTC token holders, using three distinct components; the GTC token, governance module, and Timelock. Together, these contracts allow the commGTCty to propose, vote, and implement changes to the Gitcoin protocol.

Any addresses with more than 10M GTC delegated to it may propose governance actions, which contain finished, executable code. When a proposal is created, the commGTCty can cast their votes during a 3 day voting period. If a majority, and at least 4M votes are cast for the proposal, it is queued in the Timelock, and may be executed in a minimum of 2 days.
...
```
Delegate votes from the sender to the delegatee. Users can delegate to 1 address at a time, and the number of votes added to the delegatee’s vote count is equivalent to the balance of GTC in the user’s account. Votes are delegated from the current block and onward, until the sender delegates again, or transfers their GTC.
...
| expiry         | `uint`    | The time when the signature expires. A block timestamp in seconds since the GTCx epoch.                            |
...
The sender must hold more GTC than the current proposal threshold (proposalThreshold()) as of the immediately previous block. The proposal can have up to 10 actions (based on proposalMaxOperations()).

The proposer cannot create another proposal if they currently have a pending or active proposal. It is not possible to queue two identical actions in the same block (due to a restriction in the Timelock), therefore actions in a single proposal must be GTCque, and GTCque proposals that share an identical action must be queued in different blocks.
